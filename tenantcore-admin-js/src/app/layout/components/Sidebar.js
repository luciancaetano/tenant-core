...
class Sidebar extends Component {
    ...
    activeRoute (routeName: string, props: any): any {
    ...
    render (): ReactNode {
        ...
        const badge = (badge: any): ReactNode => {
            if (badge) {
        ...
        const wrapper = (item: any): any => { return (item.wrapper && item.wrapper.element ? (React.createElement(item.wrapper.element, item.wrapper.attributes, item.name)) : item.name); };

        // nav list section title
        const title = (title: any, key: any): ReactNode => {
            const classes = classNames('nav-title', title.class);
            return (<li key={key} className={classes}>{wrapper(title)} </li>);
        };

        // nav list divider
        const divider = (divider: any, key: any): ReactNode => {
